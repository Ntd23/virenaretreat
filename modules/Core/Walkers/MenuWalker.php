<?php
	...
	class MenuWalker
	{
		...
		public function generate()
		{
			...
			if (!empty($items)) {
				echo '<ul class="main-menu menu-generated nav-header">';
		...
		public function generateTree($items = [],$depth = 0,$parentKey = '')
		{
			...
			foreach ($items as $k=>$item) {
				...
				printf('<a  target="%s" href="%s" style="font-size: 12px; padding-left:14px; padding-right:14px">%s</a>', e($item['target']), e($url), clean($item['name']));
				if (!empty($item['children'])) {
